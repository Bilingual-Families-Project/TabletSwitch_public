...
exclusion_LEQ <- combined_LEQ %>%
  ...
  mutate(
    ## preterm
    preterm = case_when(born_at_how_many_weeks < 37 ~ 1,
                        TRUE ~ 0), # 1 = preterm and to be exclude; 0 = fullterm
    ## vision, hearing, or language problems
    exclusion_disability = if_else(disability == "Yes" | disability == "Oui", 1, 0),
    exclusion_health = if_else(health_problem == "Yes" | health_problem == "Oui", 1, 0),
  ...
  select(subject_id, preterm, exclusion_disability, exclusion_health, 
...
write.csv(exclusion_LEQ, here("output/exclusion_LEQ.csv"))
```
