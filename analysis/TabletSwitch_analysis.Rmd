...
merged_data_clean <- merged_data_clean %>%
  # exclude preterm
  filter(exclude_preterm != 1 | is.na(exclude_preterm)) %>%
  # exclude those who doesn't fit the language proficiency criteria
  anti_join(exclude_proficiency, by = "subject_id")
...
plot_mean_prop <- mean_prop_by_condition %>%
  ...
  theme(panel.spacing.x = unit(2.5, "lines"),
        text = element_text(size=12)) +
  labs(title = "Mean proportion of accuracy by condition (Concordia N=31; Princeton N=8)")
