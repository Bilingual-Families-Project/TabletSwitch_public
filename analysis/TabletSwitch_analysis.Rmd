...
check_language_proficiency <- merged_data_clean %>%
  ...
  mutate(exclude_fam_proficiency = 
...
write.csv(individual, "individual_result.csv", row.names=F)
...
write.csv(individual_language, "individual_language_result.csv", row.names=F)
...
write.csv(mean_prop, "mean_prop_by_condition.csv", row.names=F)
...
plot_mean_prop <- mean_prop_by_condition %>%
  ...
  theme(panel.spacing.y = unit(1, "lines"),
        text = element_text(size=12)) +
  labs(title = "Mean proportion of accuracy by condition")
